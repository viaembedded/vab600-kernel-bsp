...
#ifdef USE_SCL
extern int ge_stretch_blit_scl(ge_info_t *geinfo,
		const ge_surface_t *src, const ge_surface_t *dst,
		ge_rect_t *sr, ge_rect_t *dr);
#endif
...
int ge_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case GEIO_ALLOW_PAN_DISPLAY:
		...
		break;
#ifdef USE_SCL
	case GEIO_STRETCH_BLIT_SCL:
		ret = get_args(args, (void *)arg, 4);
		if (ret == 0) {
			ge_surface_t s;
			ge_surface_t d;
			ge_rect_t sr;
			ge_rect_t dr;
			copy_from_user((void *)&s, (void *)args[0], sizeof(ge_surface_t));
			copy_from_user((void *)&d, (void *)args[1], sizeof(ge_surface_t));
			if (args[2])
				copy_from_user((void *)&sr, (void *)args[2], sizeof(ge_rect_t));
			if (args[3])
				copy_from_user((void *)&dr, (void *)args[3], sizeof(ge_rect_t));
			ret = ge_stretch_blit_scl(geinfo, &s, &d,
				args[2] ? &sr : NULL, args[3] ? &dr : NULL);
		}
		break;
#endif /* USE_SCL */
	default:
