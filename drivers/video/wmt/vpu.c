...
void vpu_set_direct_path(vpp_video_path_t mode)
{
	unsigned int reg;
	...
	vppif_reg32_out(REG_VPU_DIRPATH,reg);
}

vpp_video_path_t vpu_get_direct_path(void)
{
	unsigned int reg;

	reg = vppif_reg32_in(REG_VPU_DIRPATH);
	if( reg & BIT0 )
		return VPP_VPATH_VPU;
	if( reg & BIT8 )
		return VPP_VPATH_VPU_GE;
	return VPP_VPATH_GOVW;
}
...
void vpu_set_timing(vpp_clock_t *timing,unsigned int pixel_clock)
{
	...
	vppif_reg32_write(VPU_READCYC_1T,(timing->read_cycle)? 0:1);	
...
void vpu_set_filter_mode(vpp_filter_mode_t mode,int enable)
{
	if( (mode!=0) && enable ){
		if( vppif_reg32_read(VPU_V_SCALEUP) || vppif_reg32_read(VPU_H_SCALEUP) ){
			DPRINT("[VPU] *W* filter can't work w scale\n");
		}
		vppif_reg32_write(VPU_DEBLOCK_ENABLE,0);
	...
	switch( mode ){
		...
		case VPP_FILTER_DEBLOCK: // deblock
			vppif_reg32_write(VPU_DEBLOCK_ENABLE,enable);
			break;
		case VPP_FILTER_FIELD_DEFLICKER: // field deflicker
			vppif_reg32_write(VPU_FIELD_DEFLICKER,enable);
			break;
		case VPP_FILTER_FRAME_DEFLICKER: // frame deflicker
			vppif_reg32_write(VPU_FRAME_DEFLICKER,enable);	
			break;
	}
}

vpp_filter_mode_t vpu_get_filter_mode(void)
{
	if( vppif_reg32_read(VPU_V_SCALEUP) || vppif_reg32_read(VPU_H_SCALEUP) ){
		return VPP_FILTER_SCALE;
	}
	if( vppif_reg32_read(VPU_DEBLOCK_ENABLE) )
		return VPP_FILTER_DEBLOCK;

	if(	vppif_reg32_read(VPU_FIELD_DEFLICKER) )
		return VPP_FILTER_FIELD_DEFLICKER;

	if( vppif_reg32_read(VPU_FRAME_DEFLICKER) )
...
void vpu_r_set_fb_addr(unsigned int y_addr,unsigned int c_addr)
{
	...
	DBG_DETAIL("y 0x%08x,c 0x%08x\n",y_addr,c_addr);

	offset_y = offset_c = 0;
	line_y = line_c = vppif_reg32_read(VPU_R_YBUFWID);
	...
	vppif_reg32_out(REG_VPU_R_Y1SA,y_addr+offset_y);
	vppif_reg32_out(REG_VPU_R_C1SA,c_addr+offset_c);
...
void vpu_reg_dump(void)
{
	...
	DPRINT("rd cyc %d,1T %d\n",vppif_reg32_read(VPU_TG_RDCYC),vppif_reg32_read(VPU_READCYC_1T));
	...
	DPRINT("---------- VPU filter ----------\n");
	DPRINT("DEBLOCK %d,boundary 1st 0x%x,2nd 0x%x\n",vppif_reg32_read(VPU_DEBLOCK_ENABLE),
		vppif_reg32_read(VPU_1ST_LAYER_BOUNDARY),vppif_reg32_read(VPU_2ND_LAYER_BOUNDARY));
	DPRINT("FIELD DEFLICKER %d,up %s down,thr Y %d,C %d\n",vppif_reg32_read(VPU_FIELD_DEFLICKER),vppif_reg32_read(VPU_FIELD_DEFLICKER)?"&":"or",
		vppif_reg32_read(VPU_FIELD_FILTER_Y_THD),vppif_reg32_read(VPU_FIELD_FILTER_C_THD));
	DPRINT("FRAME DEFLICKER %d,%s,2^%d,scene chg %d\n",vppif_reg32_read(VPU_FRAME_DEFLICKER),vppif_reg32_read(VPU_FRAME_FILTER_RGB)?"RGB":"Y",
		vppif_reg32_read(VPU_FRAME_FILTER_SAMPLER),vppif_reg32_read(VPU_FR_FILTER_SCENE_CHG_THD));
...
// 		govw_set_int_enable(VPP_FLAG_DISABLE,VPP_INT_ERR_GOVW_TG);
		...
		govw_set_mif_enable(vpu_scale_govw_mif_en);
...
void vpu_sclw_init(void *base)
{
	...
	vppif_reg32_write(VPU_1ST_LAYER_BOUNDARY,48);
	vppif_reg32_write(VPU_2ND_LAYER_BOUNDARY,16);

	vppif_reg32_write(VPU_FIELD_FILTER_Y_THD,8);
	vppif_reg32_write(VPU_FIELD_FILTER_C_THD,8);
	vppif_reg32_write(VPU_FIELD_FILTER_CONDITION,0);
	
	vppif_reg32_write(VPU_FRAME_FILTER_RGB,0);
	vppif_reg32_write(VPU_FRAME_FILTER_SAMPLER,14);
	vppif_reg32_write(VPU_FR_FILTER_SCENE_CHG_THD,32);
}
