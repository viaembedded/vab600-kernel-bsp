...
static int i2c_wmt_wait_bus_not_busy(void)
{
	...
	while (1) {
		...
		if (cnt > MAX_BUS_READY_CNT) {
			...
			if(timeout>=3)
			{
				printk("i2c_err 0: time-out for too much time, abort waiting!\n");
				break;
			}
			timeout++;
		}
...
{
	printk("i2c2 resume\n");
	GPIO_CTRL_GP21_I2C_BYTE_VAL &= ~(BIT4 | BIT5);
	GPIO_PULL_EN_GP21_I2C_BYTE_VAL |= (BIT4 | BIT5);
	GPIO_PULL_CTRL_GP21_I2C_BYTE_VAL |= (BIT4 | BIT5);
...
static int __init i2c_adap_wmt_init(void)
{
	...
	if (is_master == 1) {
		...
		auto_pll_divisor(DEV_I2C2, CLK_ENABLE, 0, 0);
		auto_pll_divisor(DEV_I2C2, SET_DIV, 2, 20);/*20M Hz*/
		*(volatile unsigned char *)CTRL_GPIO21 &= ~(BIT4 | BIT5);
		*(volatile unsigned char *)PU_EN_GPIO21 |= (BIT4 | BIT5);
		*(volatile unsigned char *)PU_CTRL_GPIO21 |= (BIT4 | BIT5);
		i2c.regs->cr_reg  = 0 ;
		i2c.regs->div_reg = APB_96M_I2C_DIV ;
		i2c.regs->isr_reg = I2C_ISR_ALL_WRITE_CLEAR ;   /* 0x0007*/
		i2c.regs->imr_reg = I2C_IMR_ALL_ENABLE ;        /* 0x0007*/
