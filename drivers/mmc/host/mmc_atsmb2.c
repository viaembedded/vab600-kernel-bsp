...
static void atsmb2_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_OFF) {
		if (MMC2_DRIVER_VERSION == MMC_DRV_3481) {
			...
			*ATSMB2_BUS_MODE &= ~(ATSMB_CST);
            ...
            if (SD2_function == SDIO_WIFI)
                ...
                GPIO_CTRL_GP25_SD2WRSW_BYTE_VAL|= GPIO_SD2_POWER;
                GPIO_OC_GP25_SD2WRSW_BYTE_VAL|= GPIO_SD2_POWER;
            }
            
			/*set internal pull up*/
            GPIO_PULL_CTRL_GP25_SD2WRSW_BYTE_VAL |= GPIO_SD2_POWER;
			/*set internal pull enable*/
            GPIO_PULL_EN_GP25_SD2WRSW_BYTE_VAL |= GPIO_SD2_POWER;
			/*disable SD2 power*/
            if (SD2_function == SDIO_WIFI)
                GPIO_OD_GP0_BYTE_VAL &= ~BIT6;
            else
			    GPIO_OD_GP25_SD2WRSW_BYTE_VAL |= GPIO_SD2_POWER;

			/* Disable Pull up/down resister of SD Bus */
			GPIO_PULL_CTRL_GP24_SD2KPAD_BYTE_VAL &=  ~(GPIO_SD2_Data | GPIO_SD2_WriteProtect | GPIO_SD2_Command | GPIO_SD2_Clock);
            GPIO_PULL_CTRL_GP26_KPAD_BYTE_VAL &= ~GPIO_SD2_Data_2;
		...
		if (MMC2_DRIVER_VERSION == MMC_DRV_3481) {
           ...
           /*Disable Clock & CMD Pull enable*/
			GPIO_PULL_EN_GP24_SD2KPAD_BYTE_VAL &= ~(GPIO_SD2_Clock | GPIO_SD2_Command);
			
			/*Set CD ,WP ,DATA pin pull up*/
            if (SD2_function == SDIO_WIFI) {
                GPIO_PULL_CTRL_GP21_I2C_BYTE_VAL &= ~GPIO_SD2_CD; /*pull down card always in slot*/
                GPIO_PULL_CTRL_GP24_SD2KPAD_BYTE_VAL |= GPIO_SD2_Data;
				GPIO_PULL_CTRL_GP26_KPAD_BYTE_VAL |= GPIO_SD2_Data_2;
			    GPIO_PULL_CTRL_GP24_SD2KPAD_BYTE_VAL &= ~GPIO_SD2_WriteProtect; /*pull down R/W able*/
            } else {
            	GPIO_PULL_CTRL_GP21_I2C_BYTE_VAL |= GPIO_SD2_CD;
			    GPIO_PULL_CTRL_GP24_SD2KPAD_BYTE_VAL |= (GPIO_SD2_Data | GPIO_SD2_WriteProtect);
				GPIO_PULL_CTRL_GP26_KPAD_BYTE_VAL |= GPIO_SD2_Data_2;
            }
			
            
			/*Enable CD ,WP ,DATA  internal pull*/
			GPIO_PULL_EN_GP21_I2C_BYTE_VAL |= GPIO_SD2_CD;
			GPIO_PULL_EN_GP24_SD2KPAD_BYTE_VAL |= (GPIO_SD2_Data | GPIO_SD2_WriteProtect);
			GPIO_PULL_EN_GP26_KPAD_BYTE_VAL |= GPIO_SD2_Data_2;
    ...
    if (ios->clock == mmc->f_min) {
 		host->current_clock = auto_pll_divisor(DEV_SDMMC2, SET_DIV, 1, 390);
        DBG("[%s]ATSMB2 Host 400KHz\n", __func__);            
    }
	else if (ios->clock >= 50000000) {
		host->current_clock = auto_pll_divisor(DEV_SDMMC2, SET_DIV, 2, 45);
        DBG("[%s]ATSMB2 Host 50MHz\n", __func__);
	} else if ((ios->clock >= 25000000) && (ios->clock < 50000000)) {
        host->current_clock = auto_pll_divisor(DEV_SDMMC2, SET_DIV, 2, 24);
        DBG("[%s]ATSMB2 Host 25MHz\n", __func__);
	} else if ((ios->clock >= 20000000) && (ios->clock < 25000000)) {
        host->current_clock = auto_pll_divisor(DEV_SDMMC2, SET_DIV, 2, 20);
...
static int __init atsmb2_probe(struct platform_device *pdev)
{
    ...
    if (MMC2_DRIVER_VERSION == MMC_DRV_3481) {
        ...
        if (SD2_function == SDIO_WIFI) {
            GPIO_PULL_CTRL_GP21_I2C_BYTE_VAL &= ~GPIO_SD2_CD; /*pull down CD*/
        } else {
		    GPIO_PULL_CTRL_GP21_I2C_BYTE_VAL |= GPIO_SD2_CD; /*pull up CD*/
        }
		GPIO_PULL_EN_GP21_I2C_BYTE_VAL |= GPIO_SD2_CD;
