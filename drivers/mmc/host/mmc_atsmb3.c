...
void bcm_wlan_power_on(int on)
{
	...
	GPIO_CTRL_GP2_WAKEUP_SUS_BYTE_VAL |= SDIO_WIFI_PWR;
	msleep(10);
	...
	GPIO_PULL_EN_GP2_WAKEUP_SUS_BYTE_VAL |= SDIO_WIFI_INT;
	GPIO_OC_GP2_WAKEUP_SUS_BYTE_VAL &= ~SDIO_WIFI_INT;
	GPIO_CTRL_GP2_WAKEUP_SUS_BYTE_VAL |= SDIO_WIFI_INT;
	...
	*ATSMB3_BUS_MODE |= ATSMB_CST;			
	...
	if (on == 1) {
		mmc_add_host(mmc3_host_attr, true);	
		while (!mmc3_host_attr->card_scan_status) {
			msleep(10);
			i++;
			if (i>100) {
...
void bcm_wlan_power_off(int off)
{
	...
	*ATSMB3_BUS_MODE &= ~ATSMB_CST;
	...
	/* Disable SD3 interrupt */
	*ATSMB3_INT_MASK_1 &= ~ATSMB_SDIO_EN;

	/* WiFi Out-band INT */
	GPIO_OD_GP22_UART_0_1_BYTE_VAL &= ~BIT6;
	GPIO_OC_GP22_UART_0_1_BYTE_VAL |= BIT6;
	GPIO_CTRL_GP22_UART_0_1_BYTE_VAL |= BIT6;

	/* WiFi Wakeup INT */
	GPIO_OD_GP2_WAKEUP_SUS_BYTE_VAL &= ~SDIO_WIFI_INT;
	GPIO_OC_GP2_WAKEUP_SUS_BYTE_VAL |= SDIO_WIFI_INT;
	GPIO_CTRL_GP2_WAKEUP_SUS_BYTE_VAL |= SDIO_WIFI_INT;
...
void wmt_set_wifi_irq(int enable)
{
	...
	if (enable) {
		if (depth == 1) {
			GPIO6_INT_REQ_STS_VAL = BIT0; //Clear uart1cts int status
			UART1CTS_INT_REQ_TYPE_VAL |= BIT7; //Enable uart1cts int
		}
		depth--;
	} else {
		if (!depth) {
			UART1CTS_INT_REQ_TYPE_VAL &= ~BIT7; //Disable uart1cts int
		}
		depth++;
	}
}
...
int wmt_is_wifi_irq(void)
{
	if ((GPIO6_INT_REQ_STS_VAL & BIT0) && (UART1CTS_INT_REQ_TYPE_VAL & BIT7))
		return 1;
	else
		return 0;
}
