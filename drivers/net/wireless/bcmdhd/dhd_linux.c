...
dhd_open(struct net_device *net)
{
	...
	printf("%s : set module_remove to 0\n",__func__);
	module_remove = 0;
	if (module_remove) {
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (dhd_dpc_prio >= 0) {
		...
		PROC_START(dhd_watchdog_thread, dhd, &dhd->thr_wdt_ctl, 0, "dhd_watchdog");
	} else {
...
dhd_module_cleanup(void)
{
	...
	printf("%s : Set module_remove to 1 adn module_insert to 0\n",__func__);
	module_remove = 1; 
	module_insert = 0;
	msleep(1000);
...
dhd_module_init(void)
{
	...
	printf("%s : set module_insert to 1\n",__func__);
	module_insert = 1;
