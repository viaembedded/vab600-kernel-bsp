...
/* Modify to support VAB-600 Linux X Window touch screen function, by VIA Embedded, 2013.05.16 */
#define LINUXXWINDOW_TOUCH 1
...
#else
    input_report_key(ts_drv->input, BTN_TOUCH, 0);
    input_report_abs(ts_drv->input, ABS_PRESSURE, 0);
    input_sync(ts_drv->input);
#endif
}
...
#else
    input_report_key(ts_drv->input, BTN_TOUCH, 1);
    input_report_abs(ts_drv->input, ABS_PRESSURE, 1);
    input_report_abs(ts_drv->input, ABS_X, p.xpos);
    input_report_abs(ts_drv->input, ABS_Y, p.ypos);
    input_sync(ts_drv->input);
#endif
}
...
/* Modify to support VAB-600 Linux X Window touch screen function, by VIA Embedded, 2013.05.16 */
    ...
    input_set_abs_params(ts_drv->input, ABS_PRESSURE, 0, 1, 0, 0);
#else
...
static void vt1603_ts_reset(struct vt1603_ts_drvdata * ts_drv)
{
    ...
    vt1603_set_reg8(ts_drv, VT1603_TSPC_REG, 0x7C); /* VIA modification with longer precharge time = 0x20 for Powertip 800x480 touch panel based on VT1603a datasheet value */
    vt1603_set_reg8(ts_drv, 0xC5, 0x4C); /* VIA modification with longer precharge time = 0x20 for Powertip 800x480 touch panel based on VT1603a datasheet value */
