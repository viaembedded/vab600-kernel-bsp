...
/* ROW 4 */
#ifdef USE_HOME
#define KEYPAD_GPIO_CTRL_VAL_HOME GPIO_CTRL_GP24_SD2KPAD_BYTE_VAL
#define KEYPAD_GPIO_OC_VAL_HOME GPIO_OC_GP24_SD2KPAD_BYTE_VAL
#define KEYPAD_GPIO_ID_VAL_HOME GPIO_ID_GP24_SD2KPAD_BYTE_VAL
#define KEYPAD_GPIO_PULL_CTRL_VAL_HMOE GPIO_PULL_CTRL_GP24_SD2KPAD_BYTE_VAL
#define KEYPAD_GPIO_PULL_EN_VAL_HOME GPIO_PULL_EN_GP24_SD2KPAD_BYTE_VAL
#endif
...
{	
	//GPIO_PIN_SHARING_SEL_4BYTE_VAL &= ~KEYPAD_PIN_SHARE; // as keypadrow4
	KPADROW4_INT_REQ_TYPE_VAL |= BIT1; //falling edge irq type 
	KPADROW4_INT_REQ_TYPE_VAL &= ~(BIT0 | BIT2);
	KEYPAD_GPIO_CTRL_VAL_HOME &= ~KEYPAD_ROW4_GPIO_Pin; //enable gpio
	KEYPAD_GPIO_OC_VAL_HOME &= ~KEYPAD_ROW4_GPIO_Pin;  //as input
	KEYPAD_GPIO_PULL_CTRL_VAL_HMOE |= KEYPAD_ROW4_GPIO_Pin;
	KEYPAD_GPIO_PULL_EN_VAL_HOME |= KEYPAD_ROW4_GPIO_Pin; //disable pull
	GPIO4_INT_REQ_STS_VAL |= BIT3; /*Clear interrupt*/
	KPADROW4_INT_REQ_TYPE_VAL |= BIT7; //enable kpadrow4
}

static void wmt_control_home_irq(int on)
{
	if (on == 0)
		KPADROW4_INT_REQ_TYPE_VAL &= ~BIT7; //disable kpadrow4 irq
	else
		KPADROW4_INT_REQ_TYPE_VAL |= BIT7; //enable kpadrow4 irq
}
#endif
...
#ifdef USE_HOME
static void wmt_kpad_timeout_kpadrow4(unsigned long fcontext)
{
	DPRINTK("Start\n");
	if ((KEYPAD_GPIO_ID_VAL_HOME & BIT1) == 0) {   /*Active Low*/
...
static irqreturn_t wmt_home_key_interrupt(int irq, void *dev_id)
{
	...
	wmt_control_home_irq(0); //disable kpadrow4 irq 
	
	if (!(GPIO4_INT_REQ_STS_VAL & BIT3)) {
		wmt_control_home_irq(1); //Enable kpadrow4 irq 
		return IRQ_NONE;
	}
	  
	status = GPIO4_INT_REQ_STS_VAL;
	GPIO4_INT_REQ_STS_VAL |= BIT3; /*Clear interrupt*/

	if (GPIO4_INT_REQ_STS_VAL & BIT3)
		DPRINTK("[KPADROW4] Clear interrupt failed.\n");

	if (!(status & BIT3)) {
		printk("[%s] Dummy interrupt for kpadrow4, status = 0x%x\n", __func__,status);
		wmt_control_home_irq(1);
		goto OUT;
	} else {
		input_report_key(kpad_dev, wmt_kpad_codes[WMT_ROW4_KEY_NUM], 1); /*kpadrow4 key is pressed*/
        input_sync(kpad_dev);
    	mod_timer(&wmt_kpad_timer_row4, jiffies + wmt_kpad_timeout);
    	DPRINTK("[%s]WMT_ROW4_KEY_NUM press = %d\n", __func__,wmt_kpad_codes[WMT_ROW4_KEY_NUM]);
	}
OUT:
	DPRINTK("[%s]e\n",__func__);
	return IRQ_HANDLED;
	...
	if (status & KPSTR_DIA) {
		...
		if (rvalue & Dir_Vaild_Scan) {
			if ((rvalue & Dir_Input) == BIT0) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT0);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW0_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW0_GPIO_pin;
				if (back_menu_timeout == 0) {
					input_report_key(kpad_dev, wmt_kpad_codes[WMT_ROW0_KEY_NUM], 1); /*row0 key is pressed*/
					input_sync(kpad_dev);
					mod_timer(&wmt_kpad_timer_row0, jiffies + wmt_kpad_timeout);
			...
			if ((rvalue & Dir_Input) == BIT1) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT1);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW1_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW1_GPIO_pin;
			...
			if ((rvalue & Dir_Input) == BIT2) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT2);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW2_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW2_GPIO_pin;
			...
			if ((rvalue & Dir_Input) == BIT3) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT3);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW3_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW3_GPIO_pin;
			...
			if ((rvalue & Dir_Input) == BIT4) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT4);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW4_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW4_GPIO_pin;
				if (back_menu_timeout == 0) {
					input_report_key(kpad_dev, wmt_kpad_codes[WMT_ROW4_KEY_NUM], 1); /*row4 key is pressed*/
					input_sync(kpad_dev);
					mod_timer(&wmt_kpad_timer_row4, jiffies + wmt_kpad_timeout);
			...
			if ((rvalue & Dir_Input) == BIT5) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT5);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW5_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW5_GPIO_pin;
			...
			if ((rvalue & Dir_Input) == BIT6) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT6);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW6_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW6_GPIO_pin;
			...
			if ((rvalue & Dir_Input) == BIT7) {
				...
				kpad.regs->kpdcr &= ~ KPDCR_DEN(BIT7);
				KEYPAD_GPIO_CTRL_VAL |= KEYPAD_ROW7_GPIO_pin;
				KEYPAD_GPIO_OC_VAL &= ~KEYPAD_ROW7_GPIO_pin;
