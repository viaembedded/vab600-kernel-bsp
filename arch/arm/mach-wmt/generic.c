...
static unsigned int wmt_pllb_divisor = 1;
static unsigned int enter_wfi_flag = 0;
...
{
	volatile unsigned int ori_plla_value = 0;
	if (enter_wfi_flag) {
		if (!need_resched()) {
			ori_plla_value = *(volatile unsigned int *)(PMARM_ADDR);

			*(volatile unsigned int *)(PMARM_ADDR) = wmt_plla_divisor;
			while (*(volatile unsigned int *)(PMARM_ADDR) != wmt_plla_divisor)
				*(volatile unsigned int *)(PMARM_ADDR) = wmt_plla_divisor;

			while ((*(volatile unsigned int *)(PMCS_ADDR)) & 0x1)
				;

			asm("dsb");
			asm("wfi");

			*(volatile unsigned int *)(PMARM_ADDR) = ori_plla_value;

			while (*(volatile unsigned int *)(PMARM_ADDR) != ori_plla_value)
				*(volatile unsigned int *)(PMARM_ADDR) = wmt_plla_divisor;
			while ((*(volatile unsigned int *)(PMCS_ADDR)) & 0x1)
				;
		}
