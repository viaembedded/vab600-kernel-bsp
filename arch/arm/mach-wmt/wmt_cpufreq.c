...
int wmt_suspend_target(unsigned target, unsigned relation, unsigned is_suspend);
static int wmt_dvfs_reboot(struct notifier_block *nb, unsigned long event, void *unused)
{
    wmt_suspend_target(0, CPUFREQ_RELATION_L, 0);  /*change freq to lowest*/
    wmt_dvfs_running = 0;
...
int
wmt_suspend_target(unsigned target, unsigned relation, unsigned is_suspend)
{
	int ret = 0;
	struct wmt_dvfs_table *dvfs_tbl = NULL;
	...
	if (use_dvfs_debug)
		printk(KERN_INFO "change to %dKhz\n", ret / 1000);
...
{
	printk(KERN_INFO "xxxx wmt_cpufreq_suspend\n");
	return 0;
}

static int wmt_cpufreq_resume(struct cpufreq_policy *policy)
{
	printk(KERN_INFO "xxxx wmt_cpufreq_resume\n");
	return 0;
}
#else
...
static int __init wmt_cpufreq_driver_init(void)
{
	...
	if (((chip_id & 0xffff0000) == (WM3481_A2_ID & 0xffff0000)) &&
		((chip_id & 0xffff) < (WM3481_A2_ID & 0xffff))) {
		ret = -ENODEV;
		goto out;
	}
