...
static void wmt_cs_suspend(struct clocksource *cs)
{
    cyc_mark = wmt_timer_read_cycles(cs);
}

static void wmt_cs_resume(struct clocksource *cs)
{
    wmt_os_timer_set_counter(cyc_mark);
}
...
struct clocksource wmt_clocksource = {
    ...
    .suspend        = wmt_cs_suspend,
    .resume         = wmt_cs_resume,
};
